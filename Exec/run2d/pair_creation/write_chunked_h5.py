...
X_dict = csv_to_dict("X_dict_"+ str(idx) + ".csv")
X_tilde_dict = csv_to_dict("X_tilde_dict_" +str(idx) +  ".csv")



basedir ='/project/projectdirs/dasrepo/jpathak/iamr_expts/kolmogorov/processed_files/256/' +  str(idx) #'/project/projectdirs/dasrepo/jpathak/iamr_data/ldc/case1a'

X_dir = basedir + '/plots'
X_tilde_dir = basedir + '/refined_plots'
h5dir = '/global/cscratch1/sd/jpathak/iamr_training_pairs/kolmogorov/256/' + str(idx) + '/'

if ii == 0:
    if not os.path.exists(h5dir):
        os.makedirs(h5dir)

train_file = os.path.join(h5dir, 'training_pairs_' + str(ii)+ '_' + str(idx) +  '.h5')
...
for dictidx in range(startidx, endidx):
    ...
    if tilde_time in X_dict:
        X_tilde_array[arr_iter,:, :,:] = np.stack( (np.reshape( ad_X_tilde['x_velocity'] , (res, res)) , np.reshape(  ad_X_tilde['y_velocity'], (res, res) ) ) , axis = 0  ).astype(np.float32)
        X_filename = X_dict[tilde_time]    
        tilde_time_from_file = round(float(ds_X_tilde.current_time), 2)
...
with h5py.File(train_file, 'w') as f:
    f.create_dataset('fields', data = X_array[0:arr_iter,:,:,:])
    f.create_dataset('fields_tilde', data = X_tilde_array[0:arr_iter,:,:,:])
    f.flush()
